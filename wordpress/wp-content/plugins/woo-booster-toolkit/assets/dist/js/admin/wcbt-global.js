...
/***/ "./assets/src/js/admin/mixins/wp-dropdown-page.js":
/*!********************************************************!*\
  !*** ./assets/src/js/admin/mixins/wp-dropdown-page.js ***!
  \********************************************************/
...
/* harmony export */ });
const wcbtDropDownPage = () => {
  const dropdownWrapperNodes = document.querySelectorAll('.wcbt-wp-dropdown-page-wrapper');
  ...
  for (let i = 0; i < dropdownWrapperNodes.length; i++) {
    ...
    const dropDownNode = dropdownWrapperNode.querySelector('.wcbt-wp-dropdown-page-content .wcbt-dropdown');
    const createPageBtnNode = dropdownWrapperNode.querySelector('.wcbt-wp-dropdown-page-content > button');
    if (!createPageBtnNode) {
      continue;
    }
    const addPageInlineNode = dropdownWrapperNode.querySelector('.wcbt-quick-add-page-inline');
    const addPageActionNode = dropdownWrapperNode.querySelector('.wcbt-quick-add-page-actions');
    createPageBtnNode.addEventListener('click', function () {
      ...
      addPageInlineNode.querySelector('button').addEventListener('click', function (event) {
        ...
        if (!title) {
          alert(__('Please enter the title of the page', 'wcbt'));
        } else {
          const data = {
            title,
            post_type: 'page',
          ...
          wp.apiFetch({
            ...
            if (res.status && res.status === 'success') {
              ...
              createPageBtnNode.style.display = 'block';
              addPageActionNode.style.display = 'block';
              const selectControlNode = dropDownNode.querySelector('select.wcbt-tom-select').tomselect;
              selectControlNode.addOption({
                value: res.data.id,
                text: res.data.title
              });
              ...
              addPageActionNode.querySelector('.view-page').setAttribute('href', WCBT_GLOBAL_OBJECT.siteurl + '?page_id=' + res.data.id);
            }
          }).catch(err => {
            console.log(err);
          }).finally(() => {});
    ...
    if (selectNode) {
      selectNode.addEventListener('change', function () {
        ...
        if (value) {
          ...
          const editPageUrl = 'post.php?post=' + value + '&action=edit';
          const viewPageUrl = WCBT_GLOBAL_OBJECT.siteurl + '?page_id=' + value;
          if (editPageNode && viewPageNode) {
            editPageNode.setAttribute('href', editPageUrl);
            viewPageNode.setAttribute('href', viewPageUrl);
          } else {
            addPageActionNode.innerHTML = `<a class="edit-page" href="${editPageUrl}" target="_blank">` + __('Edit page', 'wcbt') + `</a> |
                        <a class="view-page" href="${viewPageUrl}" target="_blank">` + __('View page', 'wcbt') + `</a>`;
          }
        } else {
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (wcbtDropDownPage);
...
const wcbtMedia = () => {
  ...
  const mediaElNodes = document.querySelectorAll('.wcbt-image-info');
  if (!mediaElNodes) {
    return;
  }
  (0,delegated_events__WEBPACK_IMPORTED_MODULE_0__.on)('click', '.wcbt-image-info .wcbt-image-add,.wcbt-gallery-add', function (event) {
    event.preventDefault();
    const mediaElNode = this.closest('.wcbt-image-info');
    const multiple = mediaElNode.classList.contains('multiple');
    const wcbtUploader = wp.media({
      title: __('Select Images', 'wcbt'),
      button: {
        text: __('Use these images', 'wcbt')
      },
      multiple,
      library: {
        type: 'image'
    ...
    wcbtUploader.on('select', function () {
      const selection = wcbtUploader.state().get('selection');
      if (multiple === true) {
        ...
        for (let i = 0; i < attachmentIds.length; i++) {
          ...
          galleryPreviewHtml += `<div class="wcbt-gallery-preview" data-id="${dataId}">
						<div class="wcbt-gallery-centered"><img src="${src}" alt="#">
						<span class="wcbt-gallery-remove dashicons dashicons dashicons-no-alt"></span>
						</div></div>`;
        }
        if (!!galleryPreviewHtml) {
          mediaElNode.querySelectorAll('.wcbt-gallery-preview').forEach(e => e.remove());
          mediaElNode.querySelector('.wcbt-gallery-add').insertAdjacentHTML('beforebegin', galleryPreviewHtml);
        }
      } else {
        ...
        if (validateFile(attachment, mediaElNode)) {
          ...
          mediaElNode.querySelector('.wcbt-image-preview img').style.display = 'block';
          mediaElNode.querySelector('.wcbt-image-preview img').src = attachment.url;
        } else {
          mediaElNode.querySelector('input[type=text]').value = '';
          mediaElNode.querySelector('input[type=hidden]').value = '';
          mediaElNode.querySelector('.wcbt-image-preview img').style.display = 'none';
        }
      }
    });
    wcbtUploader.on('open', function () {
      const selection = wcbtUploader.state().get('selection');
      if (multiple === true) {
  ...
  for (let i = 0; i < mediaElNodes.length; i++) {
    ...
    const removeButtonNode = mediaElNode.querySelector('button.wcbt-image-remove');
    if (!removeButtonNode) {
      return;
    }
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (wcbtMedia);
...
const wcbtTomSelect = () => {
  const tomSelectNodes = document.querySelectorAll('select.wcbt-tom-select');
  ...
  const customSelectNodes = document.querySelectorAll('select.wcbt-custom-select, .custom-select select');
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (wcbtTomSelect);
...
"use strict";
/*!*********************************************!*\
  !*** ./assets/src/js/admin/wcbt-global.tsx ***!
  \*********************************************/
__webpack_require__.r(__webpack_exports__);
...
/* harmony import */ var _mixins_wp_dropdown_page__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./mixins/wp-dropdown-page */ "./assets/src/js/admin/mixins/wp-dropdown-page.js");
/* harmony import */ var _mixins_media__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../mixins/media */ "./assets/src/js/mixins/media.js");



document.addEventListener('DOMContentLoaded', event => {
  (0,_mixins_tom_select__WEBPACK_IMPORTED_MODULE_0__["default"])();
  (0,_mixins_wp_dropdown_page__WEBPACK_IMPORTED_MODULE_1__["default"])();
  (0,_mixins_media__WEBPACK_IMPORTED_MODULE_2__["default"])();
});
})();
