...
const THIM_THEME_VERSION = '5.5.7';
...
function thim_get_option_var_css() {
	...
	$terms  = get_terms('course_tag', array(
		'pad_counts'         => 1,
		'show_counts'        => 1,
		'hierarchical'       => 1,
		'hide_empty'         => 1,
		...
		foreach ($terms as $term) {
			...
			if(!empty($term_bg_color) ){
				$term_background[] = '.term-' . esc_html($term->slug) . '{
					background	: ' . $term_bg_color . '!important;
				}';
			}
			if(!empty($term_color) ){
				$term_background[] = '.term-' . esc_html($term->slug) . '{
					color: ' . $term_color . '!important;
				}';
			}
		}
	}
...
function thim_custom_admin_scripts() {
	...
	if( isset($_GET['demo_imported'])){
		wp_add_inline_style(
			'thim-admin-theme-style', 'body #customize-control-thim_page_builder_chosen{display:block!important;}'
		);
	}
