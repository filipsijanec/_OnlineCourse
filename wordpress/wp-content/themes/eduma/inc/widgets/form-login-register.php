...
if ( ! function_exists( "thim_sub_info_login_popup" ) ) {
	function thim_sub_info_login_popup( $text_profile = '', $text_logout = '' ) {
		...
		if ( ! class_exists( 'LearnPress' ) ) {
				...
				for ( $index = 0; $index < count( $items ); $index ++ ) {
					switch ( $items[$index] ) {
						case 'courses':
							...
							break;
						case 'orders':
							$menu_items_output .= '<li class="menu-item-orders"><a href="' . esc_url( $profile->get_tab_link( 'orders', true ) ) . '"><i class="tk tk-shopping-bag"></i>' . esc_html__( 'My Orders', 'eduma' ) . '</a></li>';
							break;
						...
						case 'certificates':
							if ( class_exists( 'LP_Addon_Certificates' ) ) {
								$menu_items_output .= '<li class="menu-item-certificates"><a href="' . esc_url( $profile->get_tab_link( 'certificates', true ) ) . '"><i class="tk tk-identification-star"></i>' . esc_html__( 'My Certificates', 'eduma' ) . '</a></li>';
							}
							break;
						case 'wishlist':
			...
			$html .= '<li class="menu-item-log-out">' . '<a href="' . wp_logout_url( home_url() ) . '"><i class="tk tk-alternate-sign-out"></i>' . wp_kses_post( $text_logout ) . '</a></li>';
			$html .= '</ul>';
		}
