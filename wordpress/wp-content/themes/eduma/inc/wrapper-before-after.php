		...
		if ( is_page() || is_single() || $get_post_type == "lp_collection" ) {
			...
			if ( is_page() ) {
				...
				if ( get_the_ID() == get_option( 'woocommerce_cart_page_id' ) ||
					...
					is_post_type_archive( 'lp_collection' ) ||
...
add_action( 'woocommerce_before_main_content', 'thim_wrapper_div_open', 1 );
add_action( 'woocommerce_before_main_content', 'thim_wapper_page_title', 2 );
add_action( 'woocommerce_before_main_content', 'thim_wrapper_loop_start', 10 );
//add thim_wrapper_loop_end
add_action( 'woocommerce_after_main_content', 'thim_wrapper_loop_end', 50 );
add_action( 'woocommerce_after_main_content', 'thim_wrapper_div_close', 51 );
