	...
	function thim_add_to_cart_message( $message, $product_id ) {
		...
		if ( ! empty( $title ) ) {
			...
			if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
				...
				$message = sprintf( '<a href="%s" class="button wc-forward">%s</a> <span>%s</span>', esc_url( wc_get_cart_url() ), esc_html__( 'View Cart', 'eduma' ), esc_html( $added_text ) );
			}
		}
...
if ( ! function_exists( "thim_get_instructors" ) ) {
	function thim_get_instructors( $ins = false, $vc = false ) {
        ...
        // Check load one time on a request
		if ( ! is_null( $load ) ) {
			return $load;
		}
		...
		if ( is_admin() ) {
			global $pagenow;
            // Check only load on widgets WP, on siteOrigin, on vc, on elementor
			if ( $pagenow === 'widgets.php'
			     || ( $pagenow === 'admin-ajax.php'
			          && isset( $_REQUEST['action'] )
			          && ( $_REQUEST['action'] === 'so_panels_widget_form'
			               || $_REQUEST['action'] === 'vc_edit_form' ) )
				...
				if ( $users_by_role ) {
					foreach ( $users_by_role as $user ) {
						...
						if ( $vc == true && !class_exists( '\Thim_EL_Kit\Functions' ) ) {
...
function thim_custom_internal_css() {
	...
	if ( get_theme_mod( 'thim_body_custom_class', false ) == 'demo-online-learning' ) {
		echo "\r\n/** CSS Demo Online Learning */\r\n";
		include_once THIM_DIR . "/assets/css/libs/demo-online-learning.css";
	}
	if ( get_theme_mod( 'thim_body_custom_class', false ) == 'demo-education-news' ) {
		echo "\r\n/** CSS Demo Education News */\r\n";
		include_once THIM_DIR . "/assets/css/libs/demo-education-news.css";
	}
...
if ( ! function_exists( "thim_wprm_recipe_ingredients_shortcode_checkbox" ) ) {
	function thim_wprm_recipe_ingredients_shortcode_checkbox( $ingredient_output ) {

		return '<input type="checkbox" class="wprm-checkbox" >' . $ingredient_output;
	}

	add_filter( 'wprm_recipe_ingredients_shortcode_checkbox', 'thim_wprm_recipe_ingredients_shortcode_checkbox' );
}

if ( ! function_exists( "thim_wprm_recipe_adjustable_servings_shortcode" ) ) {
	function thim_wprm_recipe_adjustable_servings_shortcode( $adjustable_servings ) {
		$adjustable_servings = '<div class="wprm-recipe-adjustable-servings-container wprm-toggle-container wprm-block-text-bold wprm-recipe-servings-toggle">
		<h5>'. esc_html__( 'Serving', 'eduma' ) .'</h5>
		<button href="#" class="wprm-recipe-adjustable-servings wprm-toggle wprm-toggle-active" data-value="1">1x</button>
		<button href="#" class="wprm-recipe-adjustable-servings wprm-toggle" data-value="2">2x</button>
		<button href="#" class="wprm-recipe-adjustable-servings wprm-toggle" data-value="3">3x</button>
		<button href="#" class="wprm-recipe-adjustable-servings wprm-toggle" data-value="4">4x</button></div>';

		return $adjustable_servings;
	}

	add_filter( 'wprm_recipe_adjustable_servings_shortcode', 'thim_wprm_recipe_adjustable_servings_shortcode' );
}

if ( ! function_exists( "thim_dynamic_tags_item_terms" ) ) {
	function thim_dynamic_tags_item_terms ( $options ) {
		$options['portfolio_category'] = esc_html__( 'Portfolio categories', 'eduma' );

		return $options;
	}

	add_filter( 'thim-ekits\dynamic-tags\item-terms', 'thim_dynamic_tags_item_terms' );
}

if ( ! function_exists( "thim_thimpress_event_l18n" ) ) {
	function thim_thimpress_event_l18n ( $array ) {
		$array['l18n'] = array(
			'labels'  => array(
	...
	add_filter( 'thimpress_event_l18n', 'thim_thimpress_event_l18n' );
}

function eduma_font_icon( $icon_name = 'home' ) {
	if ( thim_is_new_learnpress( '4.2.7.2' ) ) {
		return esc_attr( 'lp-icon-' . $icon_name );
	} else {
		return esc_attr( 'tk tk-' . $icon_name );
	}
}
